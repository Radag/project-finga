...
(function ($) {
    ...
    $.fn.pwi = function (opts) {
        ...
        function photo(j, hidden, username) {
            ...
            if (hidden)
            {
                $html = $("<div class='gallery_preview' style='display: none'/>");
                $html.append("<a href='" + j.media$group.media$thumbnail[1].url + "' rel='image_gallery' title='" + $d + "'></a>");
            ...
            else
            {
                ...
                $html = $("<div class='gallery_preview' style='height:" + (settings.thumbSize + (settings.showPhotoCaption ? 15 : 1)) + "px;" + (settings.thumbAlign == 1 ? "width:" + (settings.thumbSize + 1) + "px;" : "") + "cursor: pointer;'/>");
                $html.append("<div class='gallery_image'><a href='" + j.media$group.media$thumbnail[1].url + "' rel='image_gallery' title='" + $d + "'><img src='" + j.media$group.media$thumbnail[0].url + "'/></a></div>");
                if(settings.showPhotoDownloadPopup) {
                    var $downloadDiv = $("<div style='display: none'/>");
                    $downloadDiv.append("<a class='downloadlink' href='" + j.media$group.media$content[0].url + "'/>");
                    $html.append($downloadDiv);
        ...
        function albums(j) {
            ...
            var inlast = location.href.lastIndexOf('/');
            var year = location.href.substr(inlast+1);
            var re = new Array();
            for(var key in j.feed.entry) {
                var t = j.feed.entry[key].published.$t;
                if (t.substr(0,4) != year) {
                    re.push(key);
                } 
            }
            re = re.reverse();
            for(u=0;u<re.length;u++){
                j.feed.entry.splice(re[u],1);
            }
                ...
                if ((($.inArray(j.feed.entry[i].gphoto$name.$t, settings.albums) > -1) || 
                    ...
                    if ($keywordMatch == true) {
                        ...
                        if (settings.showAlbumThumbs) {
                            $scAlbum = $("<div class='gallery_preview' " + (settings.albumThumbAlign == 1 ? "width:" + (settings.albumThumbSize + 1) + "px;" : "") + "cursor: pointer;'/>");
                        } else {
                            $scAlbum = $("<div class='gallery_preview' style='cursor: pointer;'/>");
                        }
                        ...
                        if (settings.showAlbumThumbs) {
                            $scAlbum.append("<div class=\"gallery_image\"><img src='" + $thumb + "'/></div>");
                        }
                        if (settings.showAlbumTitles) {
                            $item_plural = (j.feed.entry[i].gphoto$numphotos.$t == "1") ? false : true;
                            $scAlbum.append('<p class="gallery_description"><a href="" class="gallery_description">' + ( (j.feed.entry[i].title.$t.length > settings.showAlbumTitlesLength) ? j.feed.entry[i].title.$t.substring(0, settings.showCaptionLength) : j.feed.entry[i].title.$t) + '</a></p><p class="gallery_description">' + (settings.showAlbumdate ? formatDate(j.feed.entry[i].gphoto$timestamp.$t) : "") + (settings.showAlbumPhotoCount ? "&nbsp;&nbsp;&nbsp;&nbsp;" + j.feed.entry[i].gphoto$numphotos.$t + " " + ($item_plural ? settings.labels.photos :  settings.labels.photo) : "" + '</p>'));
                        }
...
;
    ...
    $.fn.pwi.defaults = {
        ...
        albumThumbSize: 175, //-- specify thumbnail size of albumthumbs (default: 72, cropped not supported, supported cropped/uncropped: 32, 48, 64, 160 and uncropped only: 72, 144, 200, 288, 320, 400, 512, 576, 640, 720, 800) 
